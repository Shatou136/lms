...
//       const fileSizeToBig = fileRejection.find((rejection) =>rejection.errors
...
//     )}
//     >
//       <CardContent className="flex items-center justify-center h-full z-full p-4">
//         <input {...getInputProps()} />
//        {renderContent()}
//       </CardContent>
//     </Card>
...
// We pass currentObjectUrl as a parameter to avoid stale closure issues
const uploadFile = useCallback(async(file: File, currentObjectUrl?: string) => {
  setFileState((prev) => ({
    ...prev,
    uploading: true,
    progress: 0,
  ...
  try {
    ...
    if(!presignedResponse.ok) {
      ...
      // Revoke the stale blob URL so the dropzone re-enables for retry
      ...
      setFileState((prev) => ({
        ...
        progress: 0,
        error: true,
        objectUrl: undefined, // re-enables the dropzone
        file: null,           // clears stale file reference
      }));
...
const onDrop = useCallback((acceptedFiles: File[]) => {
  if(acceptedFiles.length > 0) {
    ...
    // Create blob URL and store it so we can pass it to uploadFile
    ...
    // Pass blobUrl directly to avoid stale closure inside uploadFile
    uploadFile(file, blobUrl);
  }
  }, [fileState.objectUrl, uploadFile, fileTypeAccepted]
);
