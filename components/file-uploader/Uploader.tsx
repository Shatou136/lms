...
export function Uploader({ onChange, value, fileTypeAccepted }: iAppProps) {
  ...
  const [fileState, setFileState] = useState<UploaderState>({
    ...
    fileType: fileTypeAccepted,
    key: value,
    // If value exists it's a Cloudinary public_id, construct the URL
    objectUrl: value ? fileUrl : undefined,
  });
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const { signature, timestamp, folder, resourceType, cloudName, apiKey } =
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          xhr.onload = () => {
            if (xhr.status === 200) {
              ...
              // public_id is what we store in the DB — used to construct URL and delete
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      if (acceptedFiles.length > 0) {
        ...
        // Revoke old object URL if it was a local blob
        if (fileState.objectUrl && !fileState.objectUrl.startsWith("http")) {
          URL.revokeObjectURL(fileState.objectUrl);
        }
